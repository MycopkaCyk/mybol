...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ИСПРАВЛЕНО: двоеточие и user_ -> user_data
    ...
    if text == "📊 Оставить отзыв":
        ...
        if 'feedback_msg_id' in context.user_data:
            try:
                # ИСПРАВЛЕНО: messag e_id -> message_id
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('type_'):
        ...
        rating_texts = {
            ...
            # ИСПРАВЛЕНО: лишний пробел перед 3
            '3': '😐 Нормально (3/5)',
            '4': '🙂 Полезно (4/5)',
            '5': '😊 Очень полезно (5/5)'
        }
        ...
        # ИСПРАВЛЕНО: exp erience -> experience, убран пробел " " -> ""
        ...
        try:
            # ИСПРАВЛЕНО: ratingful ness -> rating_usefulness, datetim e -> datetime
            db.save_feedback(
                user_id=user.id,
            ...
            # ИСПРАВЛЕНО: f " " "{type_display} -> f"{type_display}"
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not recent:
        ...
        return
    # ИСПРАВЛЕНО: кавычки и форматирование
...
# === ПОЛУЧЕНИЕ webhook_app и ЭКСПОРТ ДЛЯ RAILWAY ===
# Это нужно делать *после* создания application и *до* экспорта app
...
# Теперь можно получить webhook_app (должно работать в v21.9)
