...
let WEBHOOK_URL = (process.env.WEBHOOK_URL || process.env.RAILWAY_STATIC_URL || "").replace(/\/$/, "").trim();
if (WEBHOOK_URL && !/^https?:\/\//i.test(WEBHOOK_URL)) WEBHOOK_URL = "https://" + WEBHOOK_URL;
...
if (!WEBHOOK_URL) {
  console.error("Задайте WEBHOOK_URL или RAILWAY_STATIC_URL (домен без https подставится сам: xxx.up.railway.app)");
  process.exit(1);
}
