...
class FeedbackDatabase:
    ...
    def create_table(self):
        ...
        # ИСПРАВЛЕНО: убраны лишние пробелы в SQL
        query = """
    ...
    def save_feedback(self, user_id, user_name, rating_usefulness, rating_experience, comment, feedback_type="general"):
        ...
        # ИСПРАВЛЕНО: убраны лишние пробелы в SQL
        query = """
                INSERT INTO feedback (user_id, user_name, rating_usefulness, rating_experience, comment, feedback_type, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?);
                """
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor = self.conn.execute(query,
                                   (user_id, user_name, rating_usefulness, rating_experience, comment, feedback_type, timestamp))
        self.conn.commit()
